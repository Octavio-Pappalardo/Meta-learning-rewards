...
from outer_loop_utils import OL_buffer, Outer_loop_advantage_agent, Outer_loop_TBPTT_PPO 
...
load_weights=False       #wether to initialize the meta agents weights with pretrained weights
...
run_name = f"ADV_{exp_name}__{model_id}" 
wandb.init(project='proyect_name',
                name= run_name,
                config=vars(config))
...
if load_weights==True:
    advantageA.load_state_dict(torch.load(initial_weights_path))

optimizer = optim.Adam(advantageA.parameters(), lr=config.learning_rate, eps=config.adam_eps)
...
model_path = "../../adv_models/model.pth"
best_model_path= f"../../adv_models/{run_name}__best_model.pth"
best_model_performance = 0 

remote_inner_loop=ray.remote(run_inner_loop) #allows for the inner loops to run on several cores paralelly
wandb.watch(advantageA , log="all" ,log_freq=100) #to see gradients of the weights as histograms in the UI.
...
for update_number in range(config.num_outer_loop_updates):
    ...
    #----------------- DATA COLLECTION -----------------------
    advantageA.to('cpu')
    torch.save(advantageA.state_dict(), model_path)
    ...
    if model_performance > best_model_performance:
        ...
        torch.save(advantageA.state_dict(), best_model_path)
    ...
    advantageA=advantageA.to(device) 

    TBPTT_PPO.update(meta_agent=advantageA,buffer=meta_buffer,dont_consider_last=config.inner_loop_conf['num_steps'])
