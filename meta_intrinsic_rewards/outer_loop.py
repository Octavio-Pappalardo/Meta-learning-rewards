...
from outer_loop_utils import OL_buffer, Outer_loop_reward_agent, Outer_loop_TBPTT_PPO 
from shared_code.shared_utils import Logger , Sampler , Tasks_batch_sampler ,Statistics_tracker
...
load_weights=False    #wether to initialize the meta agent with pretrained weights
initial_weights_path= '../path_to_model/model_name.pth' 

ol_config_setting='ml1'
il_config_setting='metaworld'
config=get_ol_config(ol_config_setting)
benchmark_name= config.benchmark_name
env_name= config.env_name
sparse_or_shaped= config.sparse_or_shaped_training_target
...
#Initialize WyB and save config. , il_config is instatiated here just for logging (not used in outer loop)
il_config=get_config(il_config_setting)
config.inner_loop_conf=vars(il_config) 
model_id=int(time.time())
run_name = f"REW_{exp_name}__{model_id}" 
...
def validation_performance(logger):
    ...
    #to resolve ties
    performance=performance + 1e-6*  np.mean(logger.validation_episodes_return[-config.num_lifetimes_for_validation:])
    return performance
...
if load_weights==True:
    rewardA.load_state_dict(torch.load(initial_weights_path))

optimizer = optim.Adam(rewardA.parameters(), lr=config.learning_rate, eps=config.adam_eps)
...
model_path = "../../i_rew_models/model.pth"
best_model_path= f"../../i_rew_models/{run_name}__best_model.pth"
best_model_performance = 0 

remote_inner_loop=ray.remote(run_inner_loop) #allows for the inner loops to run on several workers paralelly
wandb.watch(rewardA , log="all" ,log_freq=100) #to see gradients of the weights as histograms in the UI.

#------------------------------------------------------------------------------
#-------------------------------TRAIN OUTER LOOP AGENT ------------------------
start_time=time.time()
for update_number in range(config.num_outer_loop_updates):

    #----------------- DATA COLLECTION -----------------------
    rewardA.to('cpu')
    torch.save(rewardA.state_dict(), model_path)

    inputs=[ (config,il_config_setting,model_path,benchmark_name,task) for task in next(task_sampler)]
    ...
    for lifetime_data in lifetimes_buffers:
        lifetime_data.preprocess_data(data_stats=data_statistics , objective_mean=config.e_rewards_target_mean) #normalize extrinsic rewards
        lifetime_data.compute_meta_advantages_and_returns_to_go(sparse_or_shaped=sparse_or_shaped, gamma=config.meta_gamma, estimation_method=config.ae["estimation_method"],
                                                            e_lambda=config.ae["bootstrapping_lambda"], starting_n= config.ae["starting_n"], 
                                                            num_to_average_over=config.ae["num_n_step_estimates"] ,skip_rate=config.ae["skip_rate"])
    ...
    if model_performance > best_model_performance:
        ...
        torch.save(rewardA.state_dict(), best_model_path)


    # --------------  UPDATING MODEL ------------------------
    if config.anneal_lr==True:
        frac = 1.0 - update_number  / config.num_outer_loop_updates  
        lrnow = frac * config.learning_rate
        optimizer.param_groups[0]["lr"] = lrnow

    rewardA=rewardA.to(device)

    TBPTT_PPO.update(reward_agent=rewardA,buffer=meta_buffer,dont_consider_last=config.inner_loop_conf['num_steps'])
